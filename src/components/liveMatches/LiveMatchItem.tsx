...
export default function LiveMatchItem(props: Props) {
  ...
  function openModal(): import("react").MouseEventHandler<HTMLHeadingElement> | undefined {
    return ()=>{
      console.log('open');
      setIsOpen(true);
      }
  ...
  return (
    <>
      ...
      <p  onClick={openModal()} className={`text-base font-light flex justify-between`}>  { match.teams[0].name } <span>{match.score !== undefined?match.score[match.teams[0].name]:"" }</span></p>
